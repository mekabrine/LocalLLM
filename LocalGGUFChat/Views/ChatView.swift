...
struct ChatView: View {
    ...
    var body: some View {
        ...
        .toolbar {
            ToolbarItem(placement: .navigationBarTrailing) {
                Menu {
                    Button { showingChatInstructions = true } label: { Label("Chat Instructions", systemImage: "slider.horizontal.3") }
                    Button { showingModelPicker = true } label: { Label("Change Model", systemImage: "cpu") }
                    Button {
                        if let diagnostic = lastDiagnostic { UIPasteboard.general.string = diagnostic.copyText }
                    } label: { Label("Copy Last Diagnostic", systemImage: "doc.on.doc") }
                    .disabled(lastDiagnostic == nil)
                    Button { regenerateLastResponse() } label: { Label("Regenerate Last", systemImage: "arrow.clockwise") }
        ...
        .sheet(isPresented: $showingEditModal) {
            if let editingMessage {
                EditMessageModal(message: editingMessage) { PersistenceController.shared.markOutdatedAfter(message: editingMessage) }
            }
        }
        ...
        .sheet(isPresented: $showingDiagnostic) {
            if let lastDiagnostic { RuntimeDiagnosticView(diagnostic: lastDiagnostic) }
        }
        .alert(item: $confirmDeleteFromHere) { msg in
            Alert(
                ...
                primaryButton: .destructive(Text("Delete")) { PersistenceController.shared.deleteFromHere(message: msg) },
                secondaryButton: .cancel()
            )
        }
        .onAppear { generateImageInsteadOfText = generationSettings.imageGenerationEnabledByDefault }
            ...
            ZStack(alignment: .bottomTrailing) {
                ScrollView {
                    LazyVStack(spacing: 8) {
                        if let model = chat.model { modelHeader(model: model).padding(.top, 10) }
                        if hasOutdatedMessages { outdatedBanner.padding(.horizontal, 14).padding(.vertical, 6) }
                        ForEach(messages) { msg in
                            MessageRow(
                                message: msg,
                                onCopy: { UIPasteboard.general.string = msg.text ?? ""; UIImpactFeedbackGenerator(style: .light).impactOccurred() },
                                onSelectText: { activeSelection = msg; showingSelectionModal = true },
                                onEdit: { editingMessage = msg; showingEditModal = true },
                ...
                .simultaneousGesture(DragGesture().onChanged { _ in showScrollToBottom = true })
                .onChange(of: messages.count) { _ in
                    if !showScrollToBottom { withAnimation(.easeOut(duration: 0.25)) { proxy.scrollTo("BOTTOM", anchor: .bottom) } }
                }
                .onChange(of: messages.last?.text) { _ in
                    if !showScrollToBottom { withAnimation(.easeOut(duration: 0.2)) { proxy.scrollTo("BOTTOM", anchor: .bottom) } }
                }
                ...
                if showScrollToBottom {
                    Button {
                        ...
                        Image(systemName: "arrow.down").font(.headline).padding(12).background(GlassBackground(cornerRadius: 18))
                    }
                    .padding(.trailing, 16)
                    .padding(.bottom, 14)
                }
    ...
    private var composer: some View {
        VStack(spacing: 8) {
            if let errorText {
                Button { if lastDiagnostic != nil { showingDiagnostic = true } } label: {
                    HStack(spacing: 8) {
                        Image(systemName: "exclamationmark.triangle.fill")
                        Text(errorText).lineLimit(2)
                        Spacer()
            ...
            if !pendingAttachments.isEmpty { attachmentsStrip }
            composerModeRow
            ...
            HStack(alignment: .bottom, spacing: 10) {
                ...
                ZStack(alignment: .topLeading) {
                    RoundedRectangle(cornerRadius: 24, style: .continuous)
                        ...
                        .overlay(RoundedRectangle(cornerRadius: 24, style: .continuous).stroke(Color.white.opacity(generateImageInsteadOfText ? 0.28 : 0.12), lineWidth: 1))
                    if inputText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                        Text(placeholderText).foregroundColor(.secondary).padding(.leading, 16).padding(.top, 13)
                    }
                    GrowingTextEditor(text: $inputText, measuredHeight: $composerTextHeight, minHeight: composerMinHeight, maxHeight: composerMaxHeight) {
                        if canSend { send() }
                    }
                ...
                Button(action: primaryComposerAction) {
                    Image(systemName: sendButtonIcon).font(.system(size: 38)).symbolRenderingMode(.hierarchical)
                }
    ...
    private var composerModeRow: some View {
        HStack(spacing: 14) {
            Button { showingFilePicker = true } label: { Label("Files", systemImage: "doc.badge.plus") }
            Button { errorText = "Voice input needs a speech-to-text backend before it can run locally." } label: { Label("Voice", systemImage: "mic") }
            Toggle(isOn: $generateImageInsteadOfText) { Label("Images", systemImage: "photo") }
                .toggleStyle(.button)
                .tint(.accentColor)
            Spacer()
        }
                ...
                ForEach(pendingAttachments) { attachment in
                    HStack(spacing: 8) {
                        ...
                        VStack(alignment: .leading, spacing: 1) {
                            Text(attachment.title).font(.caption.weight(.semibold)).lineLimit(1)
                            Text(attachment.summary).font(.caption2).foregroundColor(StudioTheme.secondaryText).lineLimit(1)
                        }
                        Button { pendingAttachments.removeAll { $0.id == attachment.id } } label: {
                            Image(systemName: "xmark.circle.fill").foregroundColor(StudioTheme.secondaryText)
                        }
                    }
    ...
    private var placeholderText: String {
        if generateImageInsteadOfText { return "Describe an image" }
    ...
    private var sendButtonIcon: String {
        ...
        return generateImageInsteadOfText ? "exclamationmark.circle.fill" : "arrow.up.circle.fill"
    }
    ...
    private func createImageGenerationPlaceholder(for userMsg: MessageEntity, prompt: String) {
        let selectedImageModel = generationSettings.defaultImageModelID.isEmpty ? "No image model selected" : "Image model saved"
        let response = """
        Image generation is not installed yet.
        ...
        LocalLLM can save and organize image models, but it cannot render pixels until a real local image backend is added.
        """
        PersistenceController.shared.appendMessage(chat: chat, role: .assistant, text: response)
        errorText = "Image generation needs a dedicated image backend before it can render."
    }
    ...
    private func regenerateLastResponse() {
        ...
        if let last = ordered.last, last.role == MessageRole.assistant.rawValue { PersistenceController.shared.deleteFromHere(message: last) }
        let assistantMsg = PersistenceController.shared.appendMessage(chat: chat, role: .assistant, text: "")
        isGenerating = true
        errorText = nil
        lastDiagnostic = nil
    ...
    private func generateReply(into assistantEntity: MessageEntity, including userEntity: MessageEntity) async {
        ...
        do {
            ...
            let userText = userEntity.text ?? ""

            await MainActor.run {
                assistantEntity.text = thinkingText(for: effective.reasoningMode, display: effective.reasoningDisplay)
                displayDriver = ResponseDisplayDriver(message: assistantEntity, mode: liveDisplay, charactersPerSecond: typingSpeed)
            }
            ...
            let finalOutput = try await ModelFileAccess.withSecurityScopedURLAsync(bookmark: model.bookmark) { url -> String in
                ...
                let prompt = PromptBuilder.build(messages: history, systemMessage: systemMessage, effectiveSettings: effective)
                var output = try await runPrompt(prompt)

                if output.isEmpty || GenerationOutputQuality.shouldRetry(output: output, userText: userText) {
                    stage = "Retrying with repair prompt"
                    await MainActor.run { displayDriver?.updateTarget("") }
                    output = try await runPrompt(PromptBuilder.tinyAssistantRetryPrompt(userText: userText))
                }

                return output.trimmingCharacters(in: .whitespacesAndNewlines)
            }

            let textToSave = finalOutput.isEmpty ? "I could not produce a readable response. Try a smaller or different text model." : finalOutput
    ...
    private func modelHeader(model: ModelReferenceEntity) -> some View {
        ...
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(model.displayName ?? "Model").font(.subheadline.weight(.bold)).lineLimit(1)
                Text("\(ByteCountFormatter.string(fromByteCount: model.fileSize, countStyle: .file)) • \(runtime.title)")
                    .font(.caption).foregroundColor(.secondary)
            }
            Spacer()
            StudioBadge(title: generationSettings.generationMode.title, color: .accentColor)
        }
        .padding(14)
    ...
    private var outdatedBanner: some View {
        HStack(spacing: 10) {
            ...
            Text("Some messages are marked out of date due to edits.").font(.caption)
            Spacer()
            Button("Clear") {
                if let first = messages.first(where: { $0.isOutdated }) { PersistenceController.shared.deleteFromHere(message: first) }
            }
    ...
    var body: some View {
        ChatBubble(text: message.text ?? "", isUser: message.role == MessageRole.user.rawValue, isOutdated: message.isOutdated)
            .contextMenu {
                ...
                if message.role == MessageRole.assistant.rawValue { Button { onRegenerate() } label: { Label("Regenerate", systemImage: "arrow.clockwise") } }
                Divider()
                ...
                VStack(alignment: .leading, spacing: 16) {
                    StudioGlassCard(cornerRadius: 18, borderColor: StudioTheme.danger.opacity(0.35)) {
                        VStack(alignment: .leading, spacing: 8) {
                            Label("Likely Cause", systemImage: "exclamationmark.triangle.fill").font(.headline).foregroundColor(StudioTheme.danger)
                            Text(diagnostic.likelyCause).font(.subheadline)
                        }
                    }
                    diagnosticsBlock(title: "What Happened", rows: [("Stage", diagnostic.stage), ("Raw Error", diagnostic.rawError)])
                    diagnosticsBlock(title: "Model", rows: [("Name", diagnostic.modelName), ("Size", diagnostic.modelSize), ("Compatibility", diagnostic.compatibility)])
                    ...
                    StudioGlassCard(cornerRadius: 18) {
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            Text("Use a model marked Runs Now, copy models into LocalLLM/Models, lower memory/output, or choose a backend-compatible quantization.")
                                .font(.subheadline)
                                .foregroundColor(StudioTheme.secondaryText)
                        }
                    }
        ...
        StudioGlassCard(cornerRadius: 18) {
            VStack(alignment: .leading, spacing: 10) {
                ...
                ForEach(Array(rows.enumerated()), id: \.offset) { _, row in
                    VStack(alignment: .leading, spacing: 2) {
                        Text(row.0).font(.caption.weight(.semibold)).foregroundColor(StudioTheme.secondaryText)
                        Text(row.1).font(.footnote.monospaced()).textSelection(.enabled)
                    }
                }
            }
        }
