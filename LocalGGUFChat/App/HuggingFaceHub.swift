...
enum ModelRuntimeSupport: String, CaseIterable, Identifiable, Hashable {
    ...
    var title: String {
        ...
        case .unsupportedFormat: return "Needs Different Backend"
    ...
    var detail: String {
        ...
        case .needsImageBackend: return "This model can be saved, but image rendering needs a dedicated image backend."
        case .needsSpeechBackend: return "This model can be saved, but speech-to-text needs a dedicated speech backend."
        case .needsVoiceBackend: return "This model can be saved, but text-to-speech needs a dedicated voice backend."
        case .needsVisionBackend: return "This model can be saved, but vision needs a dedicated vision backend."
        case .unsupportedFormat: return "This model's format, architecture, or quantization is not supported by the current text runtime."
    ...
    static func resolve(purpose: ModelCapability, fileName: String, fileSize: Int64) -> ModelRuntimeSupport {
        ...
        case .text, .fileHelper:
            ...
            if ModelBackendCompatibility.blockingIssue(for: fileName) != nil { return .unsupportedFormat }
            ...
            if compatibility == .unsupportedArchitecture { return .unsupportedFormat }
            return .runsNow
...
enum HuggingFaceService {
    static func searchModels(query: String, purpose: ModelCapability, token: String) async throws -> [HuggingFaceRepo] {
        try await HuggingFaceAPI.searchModels(query: query, purpose: purpose, token: token)
    }

    static func repoInfo(repoID: String, token: String) async throws -> HuggingFaceRepo {
        try await HuggingFaceAPI.repoInfo(repoID: repoID, token: token)
    }
    ...
    static func downloadURL(repoID: String, filename: String) throws -> URL {
        ...
        guard let url = URL(string: "https://huggingface.co/\(repoID)/resolve/main/\(encodedFile)") else { throw URLError(.badURL) }
        return url
    }
}

struct HuggingFaceDownloadItem: Identifiable, Hashable {
    enum State: String, Hashable { case queued, downloading, saved, failed }
    let id: UUID
    let repoID: String
    let filename: String
...
final class HuggingFaceDownloadStore: ObservableObject {
    ...
    func download(repoID: String, file: HuggingFaceFile, purpose: ModelCapability, token: String) {
        ...
        Task {
            await update(id: id, state: .downloading, detail: "Downloading to LocalLLM/Models…")
            do {
                ...
                let model = try PersistenceController.shared.upsertModel(from: bookmark, displayName: destination.lastPathComponent, originalPath: destination.path, fileSize: ModelFileAccess.fileSize(at: destination))
    ...
    func clearFinished() { downloads.removeAll { $0.state == .saved || $0.state == .failed } }
